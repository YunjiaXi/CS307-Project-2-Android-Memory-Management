...
Syscall get_pagetable_layout
Syscall Number: 378
Get the page table layout 

Syscall expose_page_table
Syscall Number: 379
Map a target process's page table into current process with a virtual memory range
*/
#include <linux/mm.h>
...
int expose_page_table(pid_t pid, unsigned long fake_pgd, unsigned long page_table_addr, unsigned long begin_vaddr, unsigned long end_vaddr){
    ...
    for (tmp = target_tsk->mm->mmap; tmp; tmp = tmp->vm_next)
